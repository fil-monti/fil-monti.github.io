    ...
    //   ];
      ...
      class GeoCTMCStar {
          constructor(initialStateIndex = 0) {
              ...
              this.travelTime = 0.2;             // seconds to visually "fly" between states
          ...
          draw(ctx, color = null, fixedOffset = {x: 0, y: 0}) {
              ...
              // Draw comet trail (simple line segments in pixel space) with offset
              ...
              // Head (dot) - draw from smooth headX/headY with fixed offset
              ...
              ctx.strokeStyle = dotColor.replace('1)', '0.5)'); // Slightly transparent stroke
              ctx.lineWidth = 1.2;
              ctx.stroke();
          }
      ...
      function initAnimation() {
          ...
          tree.children.forEach((child) => {
              ...
              const seq = createSequence(tree.x, tree.y, child.x, child.y, initialSequence, null, false, goesToInternal);
          ...
          if (numTips === 3) {
              ...
              sequences.forEach(seq => {
                  ...
                  if (seq.branchIndex === 1 && !seq.parent) {
                      lineage = 'root-tip3';
                  }
              ...
              sequences.forEach(seq => {
                  ...
                  if (shouldTrack && !geoStars.find(gs => gs.sequenceId === seq.sequenceId)) {
                      ...
                      if (parentStar) {
                          ...
                          // Clone the trail ONLY if this is the same color lineage (red continuing)
                          // Don't clone if this is a new color (orange split)
                          if (color === parentColor) {
                              // Same color = continuation of same lineage (red → red)
                              // Clone the trail to keep the history
                              newStar.trail = parentStar.trail.map(p => ({ ...p }));
                          }
                          // If different color (red → orange), start with fresh trail
                      } else {
          ...
          sequences.forEach(seq => {
              ...
              if (seq.progress <= 1) {
                  ...
                  if (foundNode && foundNode.children.length > 0) {
                      ...
                      // Geo star: if this sequence has a star mid-flight, force arrival.
                      const starObj = geoStars.find(gs => gs.sequenceId === seq.sequenceId);
                      if (starObj && starObj.star) {
                          const st = starObj.star;
                          if (!st.holding) {
                              // Star is mid-transmission - complete the curved route
                              const currentT = st.t;
                              st.t = 1;
                              st.i = st.j;
                              st.holding = true;
                              st.holdRemaining = sampleExp(st.lambda);
                              ...
                              const mapWidth  = 342 * z;
                              const mapHeight = 200 * z;
                              const [x, y] = projectEquirect(geoStates[st.i].lon, geoStates[st.i].lat, offsetX, offsetY, mapWidth, mapHeight);
                              
                              // If there's a route, add trail points along the remaining curve
                              if (st.route) {
                                  // Sample points along the remaining portion of the route
                                  const numSamples = 10; // Add 10 intermediate points
                                  for (let i = 1; i <= numSamples; i++) {
                                      const t = currentT + (1.0 - currentT) * (i / numSamples);
                                      const p = evalRoute(st.route, st.easeInOut(t));
                                      st.trail.push({ x: p.x, y: p.y, age: 0 });
                                  }
                                  st.route = null;
                              } else {
                                  // No route - just add final point
